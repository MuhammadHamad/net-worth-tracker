...
export function LentList() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {lent.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">You haven’t lent anything out.</p>
        ) : (
          <div className="space-y-4">
            {pageActive.length > 0 && (
              <div className="divide-y">
                {pageActive.map((l) => <LoanRow key={l.id} loan={l} settleLabel="Mark Received" />)}
              </div>
            )}
            {pageSettled.length > 0 && (
              <div>
                {pageActive.length > 0 && (
                  <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Received / Settled</p>
                )}
                <div className="divide-y opacity-70">
                  {pageSettled.map((l) => <LoanRow key={l.id} loan={l} settleLabel="Mark Received" />)}
                </div>
              </div>
            )}
            <Pagination
