          ...
          <div className="space-y-4">
            {active.length > 0 && (
              <div className="divide-y">
                {active.map((l) => <LoanRow key={l.id} loan={l} settleLabel="Mark Received" />)}
              </div>
            )}
            {settled.length > 0 && (
              <div>
                <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Received</p>
                <div className="divide-y opacity-70">
                  {settled.map((l) => <LoanRow key={l.id} loan={l} settleLabel="Mark Received" />)}
                </div>
              </div>
            )}
          </div>
